...
fn generate_string_attribute_vertex() {
    ...
    // 1: vertex for short string that is stored inline
    {
        ...
        let vertex =
            thing_vertex_generator.create_attribute_string(type_id, short_string_bytes.as_reference(), &mut snapshot).unwrap();
    ...
    // 2: vertex for long string that does not exist beforehand with default hasher
    {
        ...
        let vertex = thing_vertex_generator.create_attribute_string(type_id, string_bytes.as_reference(), &mut snapshot).unwrap();
        let vertex_id = vertex.attribute_id().unwrap_string();
        assert!(!vertex_id.is_inline());
        assert_eq!(
...

    {
        ...
        let collide_vertex =
            thing_vertex_generator.create_attribute_string(type_id, string_collide_bytes.as_reference(), &mut snapshot).unwrap();
