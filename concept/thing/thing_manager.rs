...
impl<Snapshot: ReadableSnapshot> ThingManager<Snapshot> {
    ...
    pub fn get_relations<'this>(&'this self, snapshot: &'this Snapshot) -> RelationIterator<'_, 1> {
        ...
        let snapshot_iterator = snapshot.iterate_range(KeyRange::new_within(
    ...
    pub fn get_attributes<'this>(&'this self, snapshot: &'this Snapshot) -> AttributeIterator<'_, Snapshot, 1, 2> {
        ...
        let has_reverse_iterator = snapshot.iterate_range(KeyRange::new_inclusive(
    ...
    pub fn get_attributes_in<'this>(
        ...
        Ok(attribute_type
            ...
            .map(|value_type| {
                ...
                let attribute_iterator = snapshot.iterate_range(
                ...
                let has_reverse_iterator = snapshot.iterate_range(
            ...
            ValueType::String => {
                if AttributeID::is_inlineable(value.as_reference()) {
                    ...
                    self.vertex_generator.find_attribute_id_string_noinline(
                        attribute_type.vertex().type_id_(), value.encode_string::<256>(), snapshot,
    ...
    fn get_attribute_with_value_inline(
        ...
        snapshot.get_mapped(vertex.as_storage_key().as_reference(), |_| Attribute::new(vertex.clone()))
    ...
    pub(crate) fn has_attribute<'a>(
        ...
        let has_exists = snapshot.get_mapped(has.into_storage_key().as_reference(), |value| true)
            .map_err(|err| ConceptReadError::SnapshotGet { source: err })?
            .unwrap_or(false);
        Ok(has_exists)
    }
    ...
    pub(crate) fn get_has_unordered<'this, 'a>(
        ...
        HasAttributeIterator::new(snapshot.iterate_range(
    ...
    pub(crate) fn get_has_type_unordered<'this, 'a>(
        ...
        Ok(HasAttributeIterator::new(snapshot.iterate_range(
            KeyRange::new_within(prefix, ThingEdgeHas::FIXED_WIDTH_ENCODING)
        )))
    }
    ...
    pub(crate) fn get_has_type_ordered<'this, 'a>(
        ...
        let attributes = snapshot.get_mapped(key.as_storage_key().as_reference(), |bytes| {
    ...
    pub(crate) fn get_owners<'this, 'a>(
        ...
        AttributeOwnerIterator::new(snapshot.iterate_range(
    ...
    pub(crate) fn get_relations_roles<'this, 'a>(
        ...
        RelationRoleIterator::new(snapshot.iterate_range(
    ...
    pub(crate) fn get_role_players<'a>(
        ...
        let prefix = ThingEdgeRolePlayer::prefix_from_relation(relation.into_vertex());
        RolePlayerIterator::new(snapshot.iterate_range(
            KeyRange::new_within(prefix, ThingEdgeRolePlayer::FIXED_WIDTH_ENCODING)
        ))
    }
    ...
    pub(crate) fn relation_index_player_regenerate(
        ...
        while let Some((rp, count)) = role_player {
            ...
            if is_same_rp && total_player_count > 1 {
                ...
                snapshot.put_val(index_reverse.as_storage_key().into_owned_array(), encode_value_u64(player_repetitions));
            }
            role_player = players.next().transpose().unwrap();
        }
    }
